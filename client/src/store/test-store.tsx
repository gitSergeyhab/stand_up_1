...
export const addTodo = createAsyncThunk(
  'test/addTodo',
  async({userId, title, completed} : ClientTodo, {rejectWithValue, dispatch}) => {
    try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTodos.fulfilled, (state, action) => {
        state.isLoading = false; state.error = null; state.todos = action.payload;
      })
      .addCase(fetchTodos.rejected, (state, action) => {
        state.isLoading = false; state.error = action.payload as null | string;
      });
  }
