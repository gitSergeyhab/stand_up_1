...
const fillSpaces = (numbers: number[]) => {
  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    if (spaces.some((sp) => sp === numbers[i])) {
      result.push({num: numbers[i], type: 'num'}, {num: numbers[i] + 1, type: 'space'});
    } else {
...
export const getPages = ({count, limit, offset}: GetPageNumArgs) => {
  ...
  const [firstCentralPageX, lastCentralPageX] = [currentPage - 1, currentPage + 1];
  ...
  const afterPage = currentPage < (pageCount - 2) ? Math.ceil((currentPage + pageCount) / 2) : null;

  const pages = [firstPage, beforePage, firstCentralPage, currentPage, lastCentralPage, afterPage, lastPage].filter((p) => p);
