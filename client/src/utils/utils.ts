...
import { adaptComediansToCard, adaptEventsToCard, adaptShowsToCard } from './adapters/card-adapters';
...
export const getCardData = (data: SubEventCC[] | SubShowCC[] | SubComedianCC[], type: ContentName) => {
  switch (type) {
    case ContentName.Events:
    ...
    default: return data.map((item) => adaptComediansToCard(item as SubComedianCC));
  }
};
