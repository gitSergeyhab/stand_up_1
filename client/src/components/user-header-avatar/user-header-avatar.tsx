...
export const UserHeaderAvatar = ({user} : UserHeaderAvatarProps) => {
  ...
  const closeMenuOnPageClick = (evt: MouseEvent) => {
    if (evt.target instanceof Element && !evt.target.closest('#header-avatar')) {
      closeMenu();
    }
  };
  ...
  const userMenu = shownUserMenu ? <DropUserMenu closeMenu={closeMenu} user={user}/> : null;


  return (
    <>
      <UserAvatarBtn id={'header-avatar'} radius={40} onClick={handleClickUserMenu}>
