...
export const GridCard = ( {card}: {card: GridCardType} ) => {
  ...
  const {type, id, name, date, status, picture, extName, extId, extType, numberOfRate, rate} = card;
  ...
  const rateElement = rate ? <RateCard> <Stars/>  { roundToPoints(rate) } ({ numberOfRate }) </RateCard> : null;
