...
export const Pagination = ({count}: {count: number}) => {
  ...
  const handlePageClick = (evt: MouseEvent<HTMLAnchorElement>, targetPage: number) => {
    ...
    const fields = [{name: 'offset', value: newOffset}, {name: 'limit', value: limit}];
  ...
  const pagesElements = result.map((item) => {
    if (item.type === 'space') {
      return <SpaceLink key={item.num} to={'#'}>...</SpaceLink>;
    } else {
      return (
        <PageLink
  ...
  return (
    <PageContainer>
      <NextPrevPageLink onClick={(evt) => handlePageClick( evt, currentPage - 1)} disable={+(currentPage === 1)} key={'prev'} to={'#1'}>ᐸ</NextPrevPageLink>
      {pagesElements}
      <NextPrevPageLink onClick={(evt) => handlePageClick( evt, currentPage + 1)} disable={+(currentPage === pageCount)} key={'next'} to={`#${pageCount}`}>ᐳ</NextPrevPageLink>
    </PageContainer>
  );
};
